...
function createReservationRequest($conn, $bookId, $userId, $notes = '') {
    ...
    $stmt = $conn->prepare("
        SELECT id FROM fulfilled_bookrequests 
...
function createBookReservation($conn, $bookId, $userId, $notes = '') {
    ...
    $stmt = $conn->prepare("
        INSERT INTO fulfilled_bookrequests (book_id, user_id, priority_number, expires_at, notes)
        VALUES (?, ?, ?, ?, ?)
    ");
