...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {           
    ...
    $reviewer_name = isset($_POST['reviewer_name']) ? mysqli_real_escape_string($conn, $_POST['reviewer_name']) : 'Anonymous';
    $rating = isset($_POST['rating']) ? (int)$_POST['rating'] : 0;          
    $review = isset($_POST['review']) ? mysqli_real_escape_string($conn, $_POST['review']) : '';            
    
    if ($rating >= 1 && $rating <= 5 && !empty($review)) {                   
        // Insert into reviews_and_ratings table using reviewer_name column
        $insert = "INSERT INTO reviews_and_ratings (book_id, reviewer_name, rating, review, reviewed_at)                     
                   VALUES ($book_id, '$reviewer_name', $rating, '$review', NOW())";                   
        
        if (mysqli_query($conn, $insert)) {                          
            $message = "<div class='alert alert-success border-0 shadow-sm'> Thank you $reviewer_name! Your review has been submitted.</div>";                  
        } else {                          
            $message = "<div class='alert alert-danger border-0 shadow-sm'>Error submitting review: " . mysqli_error($conn) . "</div>";                  
        }          
    }  
}   
...
// Fetch reviews (Updated to use reviewer_name column)
$review_query = "
    SELECT rating, review, reviewed_at, reviewer_name 
...
<head>
    ...
    <style>
        ...
        .book-page-header { background: #ffffff; border-bottom: 1px solid #e2e8f0; padding: 60px 0; margin-bottom: 40px; }
        .hero-flex { display: flex; gap: 40px; align-items: center; }
        .main-book-cover { width: 200px; height: 300px; object-fit: cover; border-radius: 12px; box-shadow: 0 20px 40px rgba(0,0,0,0.1); }
        .content-card { background: #ffffff; border-radius: 20px; padding: 30px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.02); border: 1px solid #f1f5f9; margin-bottom: 24px; }
        ...
        .form-control { border-radius: 12px; padding: 12px; border: 1px solid #e2e8f0; background-color: #fcfdfe; }
        .submit-btn { background: #04054b; color: #fff; padding: 14px; border-radius: 12px; font-weight: 600; width: 100%; border: none; transition: 0.3s; }
        .submit-btn:hover { background: #000; transform: translateY(-2px); }
        .review-item { padding: 25px 0; border-bottom: 1px solid #f1f5f9; }
        .user-avatar { width: 45px; height: 45px; background: #e0e7ff; color: #08042d; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-weight: bold; }
        .rec-link { display: flex; gap: 15px; text-decoration: none; color: inherit; margin-bottom: 20px; }
        ...
        @media (max-width: 768px) { .hero-flex { flex-direction: column; text-align: center; } }
    </style>
</head>
<body>
    ...
    <nav class="auth-navbar">
        <div class="container">
            ...
            <div class="nav-left-links">
                <a href="index.php" class="auth-nav-link"><i class="fas fa-home"></i> <span>Home</span></a>
                <a href="news.php" class="auth-nav-link"><i class="fa-solid fa-newspaper"></i> <span>News</span></a>
                <a href="contact.php" class="auth-nav-link"><i class="fas fa-envelope"></i> <span>Contact</span></a>
            </div>
        </div>
    </nav>
                            ...
                            <div class="review-item">
                                <div class="d-flex gap-3">
                                    <div class="user-avatar">
                                        <?php echo strtoupper(substr($rev['reviewer_name'] ?? 'U', 0, 1)); ?>
                                    </div>
                                    <div class="flex-grow-1">
                                        <div class="d-flex justify-content-between">
                                            <h6 class="mb-1 fw-bold"><?php echo htmlspecialchars($rev['reviewer_name']); ?></h6>
